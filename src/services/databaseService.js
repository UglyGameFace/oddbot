  ...
  async findOrCreateUser(telegramId, firstName = '', username = '') {
    ...
    try {
      ...
      if (error && error.code === 'PGRST116') {
        const { data: newUser, error: insertError } = await this.client.from('users')
          .insert({ tg_id: telegramId, first_name: firstName, username })
          .select().single();
...
async getDistinctSports() {
  ...
  async getSportGameCounts() {
    ...
    try {
      ...
      return Object.entries(counts).map(([sport_title, game_count]) => ({ sport_title, game_count }));
    } catch (error) {
