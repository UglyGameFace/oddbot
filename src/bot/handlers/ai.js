...
export function registerAI(bot) {
  bot.onText(/\/parlay/, async (msg) => {
    return sendAIConfigurationMenu(bot, msg.chat.id);
  });
}
...
async function sendAISportsMenu(bot, chatId, messageId) {
  ...
  // Inline listener for sport toggles lives here to avoid cross-module tokening
  bot.once('callback_query', async (cbq) => {
    if (!cbq.data?.startsWith('cfgsp_toggle_')) return;
    try { await bot.answerCallbackQuery(cbq.id); } catch {}
    const sportKey = cbq.data.replace('cfgsp_toggle_', '');
    const cfg2 = await getAIConfig(chatId);
    const set = new Set(cfg2.sports || []);
    if (set.has(sportKey)) set.delete(sportKey);
    else set.add(sportKey);
    cfg2.sports = Array.from(set);
    await setAIConfig(chatId, cfg2);
    return sendAISportsMenu(bot, chatId, messageId);
  });
...
async function handleAIBuild(bot, chatId, cfg, messageId) {
  ...
  const last = Number(await (await import('../../services/redisService.js')).default.get(limKey));
  const now = Date.now();
  if (last && now - last < AI_BUILD_MIN_INTERVAL_MS) {
    await bot.sendMessage(chatId, '⏳ Please wait before requesting another AI build.');
    return;
  }
  (await import('../../services/redisService.js')).then(({ default: r }) => r.set(limKey, now, 'EX', Math.ceil(AI_BUILD_MIN_INTERVAL_MS / 1000)));
  ...
  try {
    ...
    // Group by game; time once per game
    const byGame = {};
